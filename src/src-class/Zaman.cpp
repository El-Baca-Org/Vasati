...
{
	// ⚡ Bolt Optimizasyonu: std::to_string ve std::string birleştirme tahsisatlarını manuel karakter arabelleği ile engelle
	int h = int(td / 60) % 12;
	int m = int(td % 60);
	char buf[6];
	int i = 0;
...
void zaman::vkt_h_v_d()
{
	...
	static const char** cached_nodes_ptr = []() {
		static const char* nodes[400] = {nullptr};
		for (pugi::xml_node pt = zaman::sehir.child("prayertimes"); pt; pt = pt.next_sibling("prayertimes")) {
			...
			if (day >= 0 && day < 400) {
				nodes[day] = pt.text().get();
			}
		}
		return nodes;
	}();
	const char* const* cached_nodes = cached_nodes_ptr;
